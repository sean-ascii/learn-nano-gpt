...
class GPT(nn.Module):
  def __init__(self, config):
    ...
    # 对于pytorch, Linear的weight shape是(output_size, input_size)
    # print(f"wte weight shape: {self.transformer.wte.weight.shape}, lm_head weight shape: {self.lm_head.weight.shape}")
  ...
  def forward(self, idx, targets=None):
    ...
    # forward teh token and position embeddings
  ...
  def from_pretrained(cls, model_type):
    ...
    sd = model.state_dict()
    sd_keys = sd.keys()
    sd_keys = [k for k in sd_keys if not k.endswith('.attn.bias')] # discar this mask / buffer
    ...
    sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.attn.masked_bias')] # ignore these param
...
if ddp:
  # use of DDP at demands CUDA, we set the device appropriately according to rank
...
for step in range(50):
  ...
  tokens_per_sec = (train_loader.B * train_loader.T * ddp_world_size) / (t1 - t0)
  if master_process:
